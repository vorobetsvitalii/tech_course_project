    ...
    ui(new Ui::AdminPage)
{
    ...
    initializeLayouts(); // Widgets initialization
    initializeButton();
    initializeSpacers();
    ...
    genericGridLayout->addLayout(topHLayout.get(), 0, 0); // Generic grid layout forming
    genericGridLayout->addLayout(configurationHLayout.get(), 1, 0);
    genericGridLayout->addLayout(menuHLayout.get(), 2, 0);
...
void AdminPage::initializeSpacers()
{
    ...
    topSpacer_2 = std::make_unique<QSpacerItem>(10, 10, QSizePolicy::Fixed, QSizePolicy::Fixed);
    // Configuration layout
    ...
    // Menu layout
    menuSpacer_1 = std::make_unique<QSpacerItem>(60, 20, QSizePolicy::Fixed, QSizePolicy::Fixed);
...
void AdminPage::topHorizontalLayout()
{
    ...
    switchPushButton->setStyleSheet("QPushButton {border: none; background-color: rgb(216, 223, 233); margin-left: 10px;}");
...
void AdminPage::configurationHorizontalLayout()
{
    ...
    cancelPushButton->setStyleSheet("QPushButton {background-color: rgb(255, 255, 255); color: rgb(255, 0, 0); border:none}");

    saveChangesButton->setText("\"Savechanges\"");
    saveChangesButton->setFixedSize(80, 30);
    saveChangesButton->setStyleSheet("QPushButton {background-color: rgb(208, 0, 0); color: rgb(255, 255, 255); border:none}");
...
void AdminPage::menuHorizontalLayout()
{
    ...
    homePushButton->setStyleSheet("QPushButton {border: none; background-color: rgb(255, 255, 255); color: rgb(255, 0, 0);}");
...
void AdminPage::primaryContentArea()
{
    ...
    contentArea->setStyleSheet("QScrollArea {border-top: 1px solid white; border-left: 2px solid rgb(211, 211, 211); border-bottom: white; border-right: white;}");
}
